# ormax/query.py
from typing import List, Optional, Dict, Any, Union, Tuple, Callable, AsyncIterator
import logging
...
class QuerySet:
    ...
    """
    __slots__ = (
        'model_class', '_database', '_where_clauses', '_where_params',
        '_order_by', '_limit', '_offset', '_select_fields', '_query_cache',
        '_distinct', '_group_by', '_having', '_having_params', '_joins',
        '_annotations', '_prefetch_related', '_select_related', '_deferred_fields',
        '_only_fields', '_using_index', '_for_update', '_timeout'
    )
    ...
    def order_by(self, *fields) -> 'QuerySet':
        ...
        for field in fields:
            ...
            if field_name not in self.model_class._fields and field_name not in self._annotations:
                raise DatabaseError(f"Field '{field_name}' does not exist on model '{self.model_class.__name__}'")
    ...
    def group_by(self, *fields) -> 'QuerySet':
        ...
        for field in fields:
            if field not in self.model_class._fields and field not in self._annotations:
                raise DatabaseError(f"Field '{field}' does not exist on model '{self.model_class.__name__}'")
    ...
    async def bulk_create(self, objects: List[Dict] | List['Model'], batch_size: int = 1000) -> List['Model']:
    ...
    async def earliest(self, *fields) -> Optional:
    ...
    async def latest(self, *fields) -> Optional:
    ...
    async def in_bulk(self, id_list: List[Any] = None, *, field_name: str = 'pk') -> Dict[Any, 'Model']:
    ...
    def extra(self, select: Dict[str, str] = None, where: List[str] = None, 
              params: List[Any] = None, tables: List[str] = None,
              order_by: List[str] = None, select_params: List[Any] = None) -> 'QuerySet':
...
class Aggregation:
    ...
    async def sum(queryset: QuerySet, field: str) -> Optional[float]:
        ...
        return result['sum_result'] if result and result['sum_result'] is not None else 0
    ...
    async def avg(queryset: QuerySet, field: str) -> Optional[float]:
        ...
        result = await queryset._database.connection.fetch_one(query, tuple(params))
        return result['avg_result'] if result and result['avg_result'] is not None else 0
    ...
    async def min(queryset: QuerySet, field: str) -> Optional[Any]:
        ...
        result = await queryset._database.connection.fetch_one(query, tuple(params))
        return result['min_result'] if result and result['min_result'] is not None else None
    ...
    async def max(queryset: QuerySet, field: str) -> Optional[Any]:
        ...
        result = await queryset._database.connection.fetch_one(query, tuple(params))
        return result['max_result'] if result and result['max_result'] is not None else None
